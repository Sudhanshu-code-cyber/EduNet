    ...
    <div class="bg-white rounded-lg shadow-lg p-6 z-10 w-11/12 sm:w-1/2 relative max-h-[90vh] overflow-y-auto">
        ...
        <form id="editNoticeForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="flex flex-col sm:flex-row gap-4">
                ...
                <div class="w-full sm:w-1/2">
                    <label class="block text-gray-700 font-medium mb-1">Posted By</label>
                    <input type="text" id="edit_posted_by" name="posted_by"
            ...
            <!-- Attachment -->
            <div class="mt-4">
                <label for="edit_attachment" class="block text-gray-700 font-medium mb-1">Attachment (optional)</label>
                ...
                <div id="existingAttachment" class="mt-2 text-sm text-gray-600 hidden">
                    Existing File: <a href="#" target="_blank" id="attachmentLink" class="text-blue-600 underline">Download</a>
                </div>
            </div>
...
<script>
    ...
    function openEditModal(notice) {
        ...
        document.getElementById('edit_posted_by').value = currentUserName;
        ...
        if (notice.attachment) {
            const fileUrl = `/storage/${notice.attachment}`;
            document.getElementById('existingAttachment').classList.remove('hidden');
            document.getElementById('attachmentLink').href = fileUrl;
            document.getElementById('attachmentLink').textContent = fileUrl.split('/').pop();
        } else {
            document.getElementById('existingAttachment').classList.add('hidden');
